...
//      console.log('this.ns', this.ns);
      ...
      if (this.expression.type == 'Template' || this.expression.type == 'ConditionalBlock') {
...
//      return serializeObject.instance(this, this.name, this.attributes, this.hooks);
      ...
      var attrCamel = this.attributes || {};
      var attr = {};

      for(key in attrCamel){
        attr[camelToDash(key)]=attrCamel[key];
      }
...
//        }
        if (key == 'content'
          || (this.name && isAttribute(view, name, key))
          || (this.name && isComponentArray(view, name, key))){
          parents[key] = attr[key];
          delete attr[key];
        }
...
function isComponentArray(view, viewName, arrayNames){
//  console.log('ddd', view.views, name, arrayNames);
  var view = view.views.find(viewName);
...
function isAttribute(view, viewName, attribName){
//  console.log('ddd', view.views, name, arrayNames);
  ...
  var attributesMap = view.attributesMap || {};

  return attributesMap[attribName];
